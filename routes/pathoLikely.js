...
router.post('/patholikely', (req, res) => {
  ...
  const callAveCount = (callback) => {
    const sqlQ = 'SELECT pathogen_res, mstPathogen_id FROM mstpathogen ORDER BY mstPathogen_id DESC LIMIT 1';
    ...
    connection.query(sqlQ, (err, rows) => {
      ...
      if (rows.length > 0) {
        qslQ = parseFloat(rows[0].pathogen_res);
        paramId = parseInt(rows[0].mstPathogen_id);
      }
